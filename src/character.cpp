...
//  sprite->createSprite(width * 3, height * 3);
  ...
  */ 
  ...
  sprite->pushSprite(fx, fy,TFT_TRANSPARENT);
  //Serial.printf("fx: %d  fy: %d \n",fx,fy);
//  sprite->deleteSprite();
  uint32_t etime = millis() - startTime;
  return etime;
}

uint32_t Character::drawBmpOnSprite(LGFX_Sprite *sp, uint16_t offx, uint16_t offy) {
  uint16_t row, col, buffidx = 0;
  uint32_t startTime = millis();
  ...
  px = pos_x - offx;
  py = pos_y - offy;
  data = image[orient][pattern];
  sx = px % width; // relative position of drawing in the sprite
  sy = py % height;
  //Serial.printf("px+sx: %d  py+sy: %d \n",px+sx,py+sy);
  for (col = 0; col < width; col++) { // For each scanline...
    for (row = 0; row < height; row++) {
      uint16_t c = pgm_read_word(data + buffidx);
      c = ((c >> 8) & 0x00ff) | ((c << 8) & 0xff00); // swap back and fore
      if (c != 0x0000) {                             // ignore background
        if (s == 1) {
//          sp->drawPixel(px + col, py + row, c);
          sp->drawPixel(px + sx + col, py + sy + row, c);
        } else {
//          sp->drawRect(px + col * s, py + row * s, s, s, c);
          sp->drawRect(px + sx + col * s, py + sy + row * s, s, s, c);
        }
      }
      buffidx += 2;
    } // end pixel
